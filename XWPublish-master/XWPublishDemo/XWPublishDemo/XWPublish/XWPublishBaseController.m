...
-(void)initPickerView{
    ...
    //添加图片提示
    addImageStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(95, 50, 300, 20)];
    addImageStrLabel.text = @"添加截图能更好的帮你反馈问题";
    addImageStrLabel.font = [UIFont systemFontOfSize:13];
...
- (void)updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(0, Y, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /4.0 +20.0)* ((int)(_arrSelected.count)/4 +1)+5.0);
}
